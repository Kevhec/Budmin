...
export default function RecentBudgets() {
  const [budgetsWithPlaceholder, setBudgetsWithPlaceholders] = useState<
    Budget[]
  >([])
  const {
    updateRecentBudgets,
  ...
  useEffect(() => {
    const getPlaceholders = () =>
      new Array(4 - recentBudgets.length).fill({
        name: "",
        startDate: new Date(),
        endDate: new Date(),
        totalAmount: 0,
  ...
  return (
    <section className="relative rounded-md mb-4 md:col-span-10 md:mb-0 md:flex md:flex-col bg-white">
      ...
      <div className="relative flex flex-col gap-2 md:grow pb-2">
        {budgetsWithPlaceholder.map((budget, i) => {
          ...
          return (
            <React.Fragment key={budget.id || fallbackId}>
              <BudgetResumeCard budget={budget} hidden={hidden} />
        ...
        {recentBudgets.length === 0 && <NoDataCard />}
      </div>
    </section>
  )
}

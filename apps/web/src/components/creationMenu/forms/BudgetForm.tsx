  ...
  const form = useForm<z.infer<typeof budgetSchema>>({
    ...
    defaultValues: {
      ...
      concurrenceDefaults: getValue(
        item?.concurrence?.defaults,
        concurrenceInit.defaults,
      ),
      concurrenceTime: getValue(
        new Date(item?.concurrence?.time || ""),
        concurrenceInit.time,
      ),
      concurrenceSteps: getValue(
        item?.concurrence?.steps,
        concurrenceInit.steps,
      ),
      concurrenceWithEndDate: getValue(
        item?.concurrence?.withEndDate ? "true" : "false",
        concurrenceInit.withEndDate,
      ),
      concurrenceType: getValue(item?.concurrence?.type, concurrenceInit.type),
      concurrenceWeekDay: getValue(
        item?.concurrence?.weekDay,
        concurrenceInit.weekDay,
      ),
      concurrenceMonthSelect: getValue(
        item?.concurrence?.monthSelect,
        concurrenceInit.monthSelect,
      ),
      concurrenceEndDate: getValue(
        new Date(item?.concurrence?.endDate || ""),
    ...
    <Form {...form}>
      ...
      >
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>{t("forms.budget.inputs.startDate.label")}</FormLabel>
              <Popover>
                  ...
                  <FormControl>
                    ...
                    >
                      {field.value ? (
                        ...
                        <span>
                          {t("forms.budget.inputs.startDate.placeholder")}
                        </span>
                      )}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="flex-1">
                  <FormLabel>
                    {t("forms.budget.inputs.endDate.label")}
                  </FormLabel>
                  <Popover>
                    <PopoverTrigger asChild>
                      <FormControl>
                        ...
                        >
                          {field.value ? (
                            ...
                            <Typography variant="span">
                              {t("forms.budget.inputs.endDate.placeholder")}
                            </Typography>
                          )}
                          <CalendarIcon className="ml-auto h-4 w-4 opacity-50" />
                        </Button>

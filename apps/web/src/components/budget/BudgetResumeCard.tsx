...
export default function BudgetResumeCard({
  ...
  const { name, startDate, endDate, totalAmount, balance } = budget
  ...
  const containerClasses = cn(
    ...
    "border-none transition-colors transition-transform shadow-none px-4 py-2 md:pt-4 relative md:pl-6 md:gap-6 md:grow md:items-center md:flex-row",
    className,
  )
  ...
  return (
    ...
    >
      ...
      />
      <CardContent
        className={cn("p-0 md:justify-center w-full", {
          "space-y-4": variant === "expanded",
        })}
      >
        <div className="relative p-0 grid grid-cols-2 gap-4 h-full">
          <div className="font-inter p-0">
            <CardTitle
              className="capitalize font-medium text-lg md:text-2xl truncate md:mb-2"
              title={name}
            >
              {name}
            </CardTitle>
          ...
          <Separator
            ...
            className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 h-5/6 md:hidden"
          />
